...
/**
 * This is the model class for table "{{%finance_application_withdrawal}}".
 *
 * @property integer $id
 ...
 */
class FinanceApplicationWithdrawal extends \yii\db\ActiveRecord
{
    ...
    public static function tableName(){
        return '{{%finance_application_withdrawal}}';
    }
    ...
    public function rules(){
	    ...
	    ['amount', 'number','min' => self::MIN_AMOUNT_WITHDRAWAL],
            ...
            ['card_holder', 'match', 'pattern'=>'/^[a-z- A-Z]+$/', 
                'message'=>'ФИО должно быть указано на английском языке как на карте'], 
    ...
    public function scenarios()
    {
        return ArrayHelper::merge(parent::scenarios(), [
            self::SCENARIO_MASTER_CARD_OTHERS => ['usercomment','amount',
                'paymentsystem_id','requisites','inn','currency','term',
                'card_holder','phone'],
    ...
    /*--------------------------INITIALING ARRAYS-----------------------------*/
    public static function paymentSystemsArray(){
        return [
    ...
    protected function getStatusHumor(){
        switch ($this->status){
            ...
            case self::STATUS_PRE_CREATED: return 'label-info';
    ...
    public function getStatus_name(){
        ...
        if(in_array($this->status, array_keys($statuses)))
            return '<p class="label '.$this->statushumor.'">'.$statuses[$this->status].'</p>';       
        return '<p class=" label label-warning">Ошибка</p>';
    }
    
    public function getPaymentSystemName(){
        $statuses = self::paymentSystemsArray();
    ...
    public function correctAmount($attribute, $params) {
        ...
        if($user->account->balance<$this->amount)
            $this->addError($attribute, 'Сумма превышает ваш текущий баланс. Рекомендуемая сумма '.
                ($user->account->balance));
        if($this->amount<self::MIN_AMOUNT_WITHDRAWAL)
            $this->addError($attribute, 'Минимальная сумма для вывода средств '.
                self::MIN_AMOUNT_WITHDRAWAL);
    }
    /*------------------------------VALIDATORS---------------------------------*/
    ...
    private function sendModerInfoMail(){
        return
            Yii::$app->mailer->compose('moderApplicationWithdrawal', ['application'=>$this])
                ...
                ->setSubject('Заявка на вывод пользовательских средств в системе ' . 
                        Yii::$app->name)
                ->send();
    }
    ...
    protected function sendConfirmEmail(){
        return 
            Yii::$app->mailer->compose('confirmApplicationWithdrawal', ['application'=>$this])
                ...
                ->setSubject('Запрос на вывод денег в системе '.
                        Yii::$app->name)
                ->send();
    }
